CREATE OR REPLACE PROCEDURE taxon_lineage_widerange (pp_output_dir IN     VARCHAR2
                                                ,pp_treefile   IN     VARCHAR2
                                                ,pp_start      IN     VARCHAR2 := 'LIFE' 
                                                ,pp_maxlevel   IN     NUMBER   := 108 
                                                )
IS
    ...
    CURSOR c_tree IS
        SELECT h.taxon_name
              ...
              ,l.parent
              ,l.name
              ,substr(LPAD (' ', (LEVEL)/2) || l.name ,1,80) scientific_name
              ,substr(decode(h.taxon_name,'SPECIES'    ,'"'||l.common_name||'"'
                                         ,'SUB-SPECIES','  ('||l.common_name||')',null),1,35) common_name
...
BEGIN
    ...
    FOR r_tree in c_tree LOOP
       ...
       /* find taxon_level of parent to decide position of l_corner and length of l_line */
        ...
        IF UPPER(r_tree.name) = UPPER(pp_start) THEN
            UTL_FILE.PUT_LINE(v_tree,lpad(r_tree.taxon_level,3,' ') ||' - '||
